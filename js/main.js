...
window.addEventListener("load", function () {
  ...
  let polygonLimitLine = new LimitPolygonDongDaDistrict(map);
  ...
  mapState.findPathMethod["AStarPlus"] = new AStarPlus(mapState.listNodeRoute,null);
  mapState.findPathMethod["SMAStar"] = new SMAStar(mapState.listNodeRoute, null);
  ...
  function findPathNow() {
    ...
    // Determine the selected pathfinding method
    let selectedMethod = mapState.curNameMeathodFindPath; // Assuming this holds the selected algorithm's name.
    let weight, maxMemory;

    if (selectedMethod === "AStar" || selectedMethod === "AStarPlus") {
        weight = parseFloat(document.getElementById('weight').value) || 0.3; // Default weight to 0.3 if not provided.
    }


  if (selectedMethod === "AStar" || selectedMethod === "AStarPlus") {
    // Set weight for AStar or AStarPlus
    mapState.findPathMethod[selectedMethod].setWeight(weight);
  }
    // Ester egg
...
document.getElementById('weight').addEventListener('change', function () {
    if (mapState.stnode.orgNode != null && mapState.endnode.orgNode != null) {
        findPathNow(); // This will use the new weight value inside findPathNow()
    }
});

document.getElementById('maxMemory').addEventListener('change', function () {
    if (mapState.stnode.orgNode != null && mapState.endnode.orgNode != null) {
        findPathNow(); // This will use the new max memory value inside findPathNow()
    }
});
